...
class OpenQuestionListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset_ids = [obj.id for obj in queryset if not obj.get_accepted_answer()]
...
class SolvedQuestionListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset_ids = [obj.id for obj in queryset if obj.get_accepted_answer()]
        queryset = queryset.filter(id__in=queryset_ids)
        return queryset
...
class TagListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Question.objects.filter(tags__slug=self.kwargs.get("slug")).all()
...
class CategoryListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Tag.objects.all().annotate(count_tags=Count("taggit_taggeditem_items"))
