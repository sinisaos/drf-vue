    ...
    <div class="col-md-8 offset-md-2" v-if="questions.length !== 0">
      <div class="col-md-12" v-for="(item, index) in questions" :key="index">
        ...
        <span v-else>
          ...
          >&ensp;Unanswered</i>&ensp;
        </span>
        <i class="fa fa-eye" aria-hidden="true" title="Views">&ensp;{{ item.views }}</i>&ensp;
        <i
          class="fa fa-comment"
          aria-hidden="true"
          title="Answers"
        >&ensp;{{ item.get_answer_count }}</i>&ensp;
        <i class="fa fa-thumbs-up" aria-hidden="true" title="Likes">&ensp;{{ item.likes }}</i>
        &ensp;
        <br />
        <hr />
      </div>
  ...
  methods: {
    getQuestions() {
      if (!this.query) {
        ...
        axios
          .get("/api/open?search=" + this.query + "&page=" + this.currentPage)
          .then(res => {
            this.questions = res.data.results;
            this.total = res.data.count;
            this.orderingId = false;
